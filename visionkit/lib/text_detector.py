...
try:
  ...
  print(f"[WARNING] spaCy not found or failed to load. Entity extraction will be unavailable. Error: {e}")
  NLP = None
...
but no OSD), 4 (assume a single column of text), 5 (assume a single uniform block of vertically aligned text), 7 (treat the image as a single text line), 8 (treat the image as a single word), 9 (treat the image as a single word in a circle), 10 (treat the image as a single character), 11 (sparse text, find as much text as possible in no particular order), 12 (sparse text with OSD), and 13 (raw line, treat the image as a single text line bypassing Tesseract-specific hacks).
    ...
    def detect_characters(
        ...
        bounding_boxes = pytesseract.image_to_boxes(
            self.image,
            config=self.config
        )
      ...
      for i, bbox in enumerate(bounding_box.splitlines()):
        if i != 0:  # Skip header line
          ...
          if len(parts) == 12:  # Ensure we have all expected parts
            ...
            if draw_boxes:
              cv2.rectangle(self.original_image, (x, y), (x + w, y + h), bounding_box_color, 2)
              cv2.putText(self.original_image, word, (x, y - adjust_text_height), font, font_scale, text_color, font_thickness, cv2.LINE_AA)
      return results, self.original_image
    ...
    def extract_entities(self, text: Optional[str] = None):
      """
    ...
    def compare_matches_knn_matcher(self, image2, form_name, no_of_feature=500, matched_amount=50, percentage_of_matches=20, draw_matches=False, draw_aligned=False):
      ...
      keypoints2, descriptors2, _ = text_form_2.detect_keypoints(features=no_of_feature, draw_keypoints=False)
    ...
    def detect_keypoints(self, features=500, draw_keypoints=False, keypoint_color=(0, 255, 0)):
        orb = cv2.ORB_create(nfeatures=features)
        keypoints, descriptors = orb.detectAndCompute(self.image, None)
        if draw_keypoints:
            self.image = cv2.drawKeypoints(self.image, keypoints, None, color=keypoint_color, flags=cv2.DrawMatchesFlags_DRAW_RICH_KEYPOINTS)
    ...
    def get_text_regions(self):
      ...
      data = pytesseract.image_to_data(self.image, config=config,
      ...
      for i in range(n):
        ...
        regions.append({"bbox": (x, y, w, h), "text": text, "conf": float(data["conf"][i])})
      return regions
    ...
    def highlight_words(self, target_words, color=(0, 255, 0), thickness=2):
      ...
      for word in words:
        if word["text"].lower() in targets:
          ...
          cv2.putText(out, word["text"], (x, max(0, y - 5)),
