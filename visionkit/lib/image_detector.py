...
class ImageDetector:
    ...
    def compare_matches_knn_matcher(self, image2, form_name, no_of_feature=500, matched_amount=50, percentage_of_matches=20, draw_matches=False, draw_aligned=False):
       # Detect keypoints
      image_detector = ImageDetector(image2)
      
      keypoints1, descriptors1, _ = self.detect_keypoints(features=no_of_feature, draw_keypoints=False)
      keypoints2, descriptors2, _ = image_detector.detect_keypoints(features=no_of_feature, draw_keypoints=False)
    ...
    def compare_matches_bf_matcher(self, image2, form_name, no_of_feature=500, matched_amount=50, percentage_of_matches=20, draw_matches=False, draw_aligned=False):     
      # Detect keypoints
      image_detector = ImageDetector(image2)
    ...
    def detect_highlighted_text(self,
      hsv_colors=[(27, 167, 251)],   # seed HSV values
      h_tol=8,
      s_tol=80,
      v_tol=80,
      show_mask=False,
    ...
    def detect_single_highlighted_text(self, image, hsv_colors=[27, 167, 251]):
    ...
    def find_contours(
      ...
      canny_threshold:list[int]=[100,100],
      debug=False, 
      dilate_merge=True,
      filter_shapes:list = [],## To detect for triangle or rectange/square shapes
      ...
      if get_canny_edges:
        cleaned = self.get_cannty_edges(low_threshold=canny_threshold[0], high_threshold=canny_threshold[1])
      else:
      ...
      contours, _ = cv2.findContours(
          cleaned,
          retrieval_type,      # only outer regions
      ...
      for cnt in contours:
          ...
          peri = cv2.arcLength(cnt, True) # Computes the perimeter of the contour and True → contour is closed
          ...
          """
          if draw_contours:
              print(f"Accepted contour with area {area} and bounding box (x={x}, y={y}, w={w}, h={h})")
              cv2.putText(countour_img, str(len(approx)), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX,1, contour_text_color, contour_text_thickness)
              
          center_x, center_y = x + (w // 2), y + (h // 2)
          cv2.rectangle(countour_img, (x, y), (x + w, y + h), contour_box_color, contour_box_thickness)
          cv2.circle(countour_img, (center_x, center_y), 5, contour_box_color, cv2.FILLED)
      ...
      if debug:
          ...
          for i, c in enumerate(contours_results):
            ...
            print(f"[{i}] Area={c['area']:.2f}, Box=({x},{y},{w},{h}), Center={c['center']}")
    ...
    def pad_to_square(self, fill: int = 0) -> np.ndarray:
      ...
      return cv2.copyMakeBorder(
          self.image, top, pad_h - top, left, pad_w - left,
    ...
    def overlay_image(self, overlay: np.ndarray, x: int, y: int, alpha: float = 1.0) -> np.ndarray:
    ...
    def compare_histograms(self, other_image: np.ndarray) -> float:
      ...
      return float(cv2.compareHist(_hist(self.image), _hist(other_image), cv2.HISTCMP_CORREL))
