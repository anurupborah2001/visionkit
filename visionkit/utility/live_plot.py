...
# Dereived from https://github.com/cvzone/cvzone/blob/master/cvzone/PlotModule.py
class LivePlot:
    """
    ...
    def __init__(
        self, w=640, h=480, yLimit=None, interval=0.001, invert=True, char="Y"
    ):
        """
    ...
    def drawBackground(self):
        ...
        cv2.line(
            self.imgPlot, (0, self.h // 2), (self.w, self.h // 2), (150, 150, 150), 2
        )
        ...
        for y in range(0, self.h, 50):
            ...
            y_label = int(
                self.yLimit[1]
                - ((y / 50) * ((self.yLimit[1] - self.yLimit[0]) / (self.h / 50)))
            )
            cv2.putText(
                self.imgPlot,
                str(y_label),
